...
class BusinessCardsAdapter(private val dataSet: ArrayList<BusinessCard>, private var context: Activity, private val controller: BusinessCardController) :
    ...
    override fun getChildView(
        ...
        if (convView != null) {
            ...
            // Set buttons visibility according to user-card relation

            convView.findViewById<FloatingActionButton>(R.id.listItemDelete).visibility = if (card.isMine) View.VISIBLE else View.GONE
            convView.findViewById<FloatingActionButton>(R.id.listItemEdit).visibility = if (card.isMine) View.VISIBLE else View.GONE
            convView.findViewById<FloatingActionButton>(R.id.listItemImage).visibility = if (card.hasImage()) View.VISIBLE else View.GONE
            convView.findViewById<ConstraintLayout>(R.id.listItemToolbar).visibility =
                if (card.hasImage() || card.isMine) View.VISIBLE else View.GONE
            if (card.isMine) {
                // Set callbacks only for users, who can edit
                convView.findViewById<FloatingActionButton>(R.id.listItemDelete).setOnClickListener {
                    controller.deleteCard(card.id, this)
                }
                convView.findViewById<FloatingActionButton>(R.id.listItemEdit).setOnClickListener {
                    controller.editCard(card, this)
                }
            }
            if (card.hasImage()) {
                convView.findViewById<FloatingActionButton>(R.id.listItemImage).setOnClickListener {
                    controller.showImage(card)
                }
            }
            convView.setBackgroundColor(Color.parseColor("#ffe0e0e0"))
        }
    ...
    override fun getGroupView(groupPosition: Int, isExpanded: Boolean, convertView: View?, parent: ViewGroup?): View {
        ...
        if (conv != null) {
            ...
            conv.setBackgroundColor(Color.parseColor(if (isExpanded) "#ffe0e0e0" else "#00ffffff"))
